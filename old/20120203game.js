...
/////////////////////////////////////////////////////////////////////////////
		...
		for( var i = 0; i < numBullets; i++ ){
			...
			bullets[i].addEventListener( 'enterframe', function (){
				...
				if( this.x < -this.width-32 || this.x > game.width+32 || this.y < -this.height-32 || this.y > game.height+32 || this.time > sec(4) ){
					this.visible = false;
					this.using = false;
					stage.removeChild( this );
				}
		...
		//特定場所狙い
		/////////////////////////////////////////////////////////////////////////////
		var enterPlaceBullet = function( x, y, tx, ty, speed ){
		...
		//自機狙い弾
		/////////////////////////////////////////////////////////////////////////////
		var enterSnipeBullet = function( x, y, speed ){
		...
		var enterNWayBullet = function( x, y, tx, ty, theta, n, speed ){
			...
			for( var i = 0; i < n; i++, rad += rad_step ){
				...
				var vx = vx0 * c - vy0 * s, vy = vx0 * s - vy0 * c;
				enterBullet( 0, x, y, vx, vy );
			}
		}
		//円形弾
...
//					enterSnipeBullet( this.x + 16, this.y + 20, 3 );
					enterNWayBullet( this.x + 16, this.y + 20, this.x + 16, this.y + 140 , 10, 3, 1 );
			...
			'Goriate':function(){
				...
				if( this.cycle == 0 ){
					if( this.y < 20 ){
						if(this.time % 5 == 0 ){
							this.y += 2;
							this.bb.y += 2;
							if( this.armL )this.armL.y += 2;
							if( this.armR )this.armR.y += 2;
						}
					}else{
						this.cycle++;
						this.vx = 1;
				...
				if( this.cycle == 1 ){
					if(this.time % 5 == 0 ){
						this.x += this.vx;
						this.bb.x += this.vx;
						if( this.armL )this.armL.x += this.vx;
						if( this.armR )this.armR.x += this.vx;
						if( this.x > 200 ){
							this.vx = -1;
						}
						if( this.x < 10 ){
				...
				if( this.def / this.defMax < 0.2 ){
					if( this.time % 20 == 0 )burn( 1, this.x+rand(this.width+16)-32, this.y+rand(this.height+16)-32 );
				}
			},
			//左腕
			'Goriate_L':function(){
