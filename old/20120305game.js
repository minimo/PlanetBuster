...
*/
		...
		stage.addEventListener( 'enterframe', function() {
			...
			if( this.time > 120 && this.time % 120 == 0  && !this.event && !this.bossBattle ){
...
//					stage.backgroundColor = "#0D2830";
				...
				if( this.time % 30 == 0 && !stage.bossBattle ){
					for( var i = 0; i < 20; i++ ){
						if( !cl[i].using )break;
					}
					if( i < 20 ){
						...
						var op = 1.3 - 1/cl[i].vy;//rand(5)/10+0.5;
						if( op > 0.8 ){
							cl[i].opacity = 0.8;
						}else{
							cl[i].opacity = op;
...
/////////////////////////////////////////////////////////////////////////////
		...
		fpsLabel.addEventListener('enterframe', function(){
			...
			var dd = new Date();
			var m = dd.getMinutes();
			this.s = dd.getSeconds();
			this.ms = m*60000+this.s*1000+dd.getMilliseconds();
			dd = null;
			//可変ＦＰＳ制御
			if( this.ms > this.bms+100 ){
				if( this.fpms < 6 && !this.boost ){
					window.clearInterval( game._intervalID );
					game._intervalID = window.setInterval(function() { game._tick() }, 1000 / 120 );
					this.boost = true;
				}
				if( this.fpms > 6 && this.boost ){
					window.clearInterval( game._intervalID );
					game._intervalID = window.setInterval(function() { game._tick() }, 1000 / 60 );
					this.boost = false;
				}
				this.bms = this.ms;
				this.fpms = 0;
...
/////////////////////////////////////////////////////////////////////////////
		...
		//払い出し
		var getEffect = fuction(){
			for( var i = 0; i < numEffects; i++ ){
				if( !effects[i].using )return effects[i];
			}
			return null;
		}
			...
			ec.addEventListener('enterframe', function() {
				if( this.wait < this.time ){
					if( player.y > this.step && this.flag ){
						player.y = ~~( (this.step - player.y ) / 10 + player.y );
...
//					enterBullet( this, BULLET_SMALL, this.x + 96, this.y + 20,  1, 3, 0 );
			...
			'SkyFish':function(){
				...
				switch( this.pattern ){
					case 0:
						if( this.y < this.ty ){
							var by = this.y;
							this.y = ~~( (this.ty - this.y ) / 20 + this.y );
			...
			'SkyBox':function(){
				...
				if( this.time % 60 == 0 ){
					enterNWayBullet( this, BULLET_SMALL, this.x+16, this.y+20, this.x+16, this.y+100, 20, 3, 3, 0 );
					enterNWayBullet( this, BULLET_SMALL, this.x+80, this.y+20, this.x+80, this.y+100, 20, 3, 3, 0 );
				}
				switch( this.pattern ){
					case 0:
						this.x = ~~( (this.tx - this.x ) / 30 + this.x );
						this.y = ~~( (this.ty - this.y ) / 30 + this.y );
						if( this.time > 90 )this.pattern++;
						break;
					case 1:
						this.x += this.vx;
						this.y -= 1;
			...
			'SkyBreaker':function(){
				...
				//回転
				if( this.pattern == 1 && this.time-this.time2 > 90 ){
					if( this.time % 60 == 0 ){
						if( this.count % 2 ){
...
//						enterNWayBullet( this, BULLET_MEDIUM1, this.x+24, this.y+60, this.x+24, this.y+100 , 20, 4, 2, 0 );
				...
				//螺旋撒き
				if( this.pattern == 2 && this.cycle == 1 && this.time-this.time2 > 90 ){
					if( this.time % 10 == 0 ){
						var rad = this.rad2;
						var tx = this.x + 76 + Math.sin( rad )*320;
						var ty = this.y + 60 + Math.cos( rad )*320;
						enterPlaceBullet( this, BULLET_MEDIUM1, this.x+76, this.y+40, tx, ty, 3, 0 );
						rad+=1.57;
						var tx = this.x + 76 + Math.sin( rad )*320;
						var ty = this.y + 60 + Math.cos( rad )*320;
						enterPlaceBullet( this, BULLET_MEDIUM1, this.x+76, this.y+40, tx, ty, 3, 0 );
				...
				if( this.pattern == 2 && this.cycle == 0 && this.time-this.time2 > 90 ){
					...
					if( this.time % 5 == 0 ){
						enterNWayBullet( this, BULLET_MEDIUM1, this.x+ 76, this.y+40, this.tx, this.ty, 30, 4, 3, 0 );
						enterNWayBullet( this, BULLET_MEDIUM1, this.x+  7, this.y+47, this.tx, this.ty, 30, 4, 3, 0 );
						enterNWayBullet( this, BULLET_MEDIUM1, this.x+145, this.y+47, this.tx, this.ty, 30, 4, 3, 0 );
						this.rad2+=1;
					}
					if( this.rad2 > 5 ){
						this.pattern--;
			...
			'SkyBreaker_C':function(){
				...
				if( this.time % 15 == 0 && this.count > 0 ){
